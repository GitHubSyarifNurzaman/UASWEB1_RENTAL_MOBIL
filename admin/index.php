...
if (!empty($_POST['nama_rental'])) {
    ...
    $row->execute($data);
    echo '<script>alert("Update Data Info Website Berhasil !");window.location="../halaman_utama.php"</script>';
    exit;
}
...
if (!empty($_POST['nama_pengguna'])) {
    ...
    $row = $koneksi->prepare($sql);
    $row->execute($data);
    echo '<script>alert("Update Data Profil Berhasil !");window.location="halaman_utama.php"</script>';
    exit;
}
?>
<!DOCTYPE html>
